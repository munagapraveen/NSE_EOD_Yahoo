...
from PySide6.QtCore import Qt, QProcess, QTimer, QSize, QDate, Signal
from PySide6.QtGui import QFont, QIcon, QColor, QTextCursor, QIntValidator, QDoubleValidator
...
class SectionCard(QGroupBox):
    def __init__(self, title, subtitle, parent=None):
        ...
        self.main_layout.setContentsMargins(10, 2, 10, 5)
        self.main_layout.setSpacing(4)
        
        if subtitle:
            sub_label = QLabel(subtitle)
            sub_label.setStyleSheet("color: #94a3b8; font-size: 10px; font-weight: normal;")
            self.main_layout.addWidget(sub_label)

class YahooNSEGUI(QMainWindow):
    stats_updated = Signal(int, str)
    ...
    def _build_tasks_section(self):
        ...
        card.main_layout.addWidget(auto_btn, 0, Qt.AlignCenter)
        ...
        """)
        card.main_layout.addWidget(self.refetch_last_cb, 0, Qt.AlignCenter)
        
        card.main_layout.addSpacing(10)
        ...
        for label, script, args in TASKS:
            ...
            card.main_layout.addWidget(btn, 0, Qt.AlignCenter)
            
        card.main_layout.addSpacing(5)
        ...
        """)
        card.main_layout.addWidget(self.apply_changes_cb, 0, Qt.AlignCenter)
        ...
        card.main_layout.addWidget(detect_btn, 0, Qt.AlignCenter)
        ...
        card.main_layout.addWidget(retry_btn, 0, Qt.AlignCenter)
    ...
    def _build_sharpe_section(self):
        ...
        self.sharpe_mcap.setValidator(QIntValidator(1, 1000000, self))
        add_row("MCAP (Cr):", self.sharpe_mcap, 1)
        
        self.sharpe_rf = QLineEdit("6.5")
        self.sharpe_rf.setValidator(QDoubleValidator(-100.0, 100.0, 2, self))
        add_row("ROC Hurdle %:", self.sharpe_rf, 2)
        
        self.sharpe_turnover = QLineEdit("1.0")
        self.sharpe_turnover.setValidator(QDoubleValidator(0.0, 100000.0, 2, self))
        ...
        self.sharpe_long.setValidator(QIntValidator(1, 120, self))
        self.sharpe_short = QLineEdit("3")
        self.sharpe_short.setValidator(QIntValidator(1, 120, self))
        ...
        card.main_layout.addWidget(run_btn, 0, Qt.AlignCenter)
    ...
    def _build_snapshot_section(self):
        ...
        self.snapshot_limit.setValidator(QIntValidator(1, 10000, self))
    ...
    def _build_query_section(self):
        ...
        card.main_layout.addWidget(self.export_query_btn, 0, Qt.AlignCenter)
    ...
    def _refresh_stats(self):
        ...
        def worker():
            try:
                ...
                self.stats_updated.emit(active_count, display_date)
            except Exception as e:
                log.error(f"Failed to refresh stats: {e}")

        threading.Thread(target=worker, daemon=True).start()

    def _update_stats_ui(self, active_count, display_date):
    ...
    def _handle_finished(self, exit_code, exit_status, label):
        ...
        if self._user_stopped:
            self.status_label.setText(f"Stopped: {label}")
            self._log(f"\n[Stopped] {label} was stopped by user.\n")
            self._user_stopped = False
        elif exit_code == 0:
